...
fun MatchesDTO.toMatchesEntity(teamId: String? = null): MatchesEntity = MatchesEntity().apply {
...
fun MatchDTO.toMatchEntity(): MatchEntity = MatchEntity().apply {
...
fun TeamInfoDTO.toTeamEmbeddedEntity(): TeamEmbeddedEntity = TeamEmbeddedEntity().apply {
...
fun RefereeDTO.toRefereeEntity(): RefereeEntity = RefereeEntity().apply {
...
fun ScoreDTO.toScoreEntity(): ScoreEntity = ScoreEntity().apply {
...
fun TimeDTO.toFullTimeEntity(): TimeEntity = TimeEntity().apply {
    away = this@toFullTimeEntity.away ?: 0
    home = this@toFullTimeEntity.home ?: 0
}

fun MatchesEntity.toCompetitionMatches(): Matches = Matches(
    id = id,
    matchesByTourCompleted = filterCompletedMatches(this),
    matchesByTourAhead = filterAheadMatches(this),
)

fun MatchesEntity.toTeamMatches(): TeamMatches = TeamMatches(
    id = id,
...
fun groupMatchesByDateCompleted(matches: List<MatchEntity>): List<Match> = matches.filter {
    it.status == "FINISHED" && it.homeTeam?.id != null && it.awayTeam?.id != null
}
    .sortedByDescending { it.utcDate }.map { it.toMatches() }

fun groupMatchesByDateAhead(matches: List<MatchEntity>): List<Match> = matches.filter {
    it.status != "FINISHED" && it.homeTeam?.id != null && it.awayTeam?.id != null
}
...
fun TeamEmbeddedEntity?.toMatchTeam(): MatchTeam = MatchTeam(
...
fun RefereeEntity.toReferee(): Referee = Referee(
    id = id,
    name = name,
...
fun TimeEntity?.toTime(): Time = Time(
    away = this?.away ?: -1,
    home = this?.home ?: -1,
)
