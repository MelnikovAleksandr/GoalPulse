...
fun TeamInfoDTO.toTeamInfoEntity(): TeamInfoEntity = TeamInfoEntity().apply {
...
fun PersonDTO?.toCoachEntity(): PersonEntity = PersonEntity().apply {
...
fun ContractDTO?.toContractEntity(): ContractEntity = ContractEntity().apply {
    start = this@toContractEntity?.start ?: ""
    until = this@toContractEntity?.until ?: ""
}

fun TeamInfoEntity.toTeamInfo(): TeamInfo = TeamInfo(
    address = address,
    area = area.toArea(),
...
fun PersonEntity?.toCoach(): Coach = Coach(
...
fun ContractEntity?.toContract(): Contract = Contract(
    start = this?.start ?: "",
    until = this?.until ?: "",
)

fun SquadByPositionEntity.toSquadByPosition(): SquadByPosition = SquadByPosition(
    position = PlayerPosition.fromValue(position),
    squad = squad?.map { it.toSquad() } ?: emptyList(),
)

fun PersonEntity.toSquad(): Squad = Squad(
