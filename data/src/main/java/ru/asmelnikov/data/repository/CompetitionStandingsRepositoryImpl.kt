...
class CompetitionStandingsRepositoryImpl(
    ...
    override suspend fun getStandingsFlowFromLocalById(compId: String): Flow<CompetitionStandings?> = withContext(
        Dispatchers.IO,
    ) {
    ...
    override suspend fun getScorersFlowFromLocal(compId: String): Flow<CompetitionScorers?> = withContext(
        Dispatchers.IO,
    ) {
    ...
    override suspend fun getAllMatchesFlowFromLocal(compId: String): Flow<Matches?> = withContext(Dispatchers.IO) {
        realmOptions.getMatchesFlowById(compId).map { it?.toCompetitionMatches() }
    }

    override suspend fun getHead2headById(matchId: Int): Resource<Head2head> = withContext(Dispatchers.IO) {
        retrofitErrorsHandler.executeSafely {
            val response =
