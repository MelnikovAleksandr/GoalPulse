...
class CompetitionsRealmOptionsImplTest {
    ...
    private suspend fun localCompetitions(): Set<Pair<Int, String>> {
        return realmOptions.getCompetitionsFlowFromLocal().first().toIdNameSet()
    }

    private suspend fun localCompetitionFields(): Set<CompetitionFields> {
        return realmOptions.getCompetitionsFlowFromLocal().first().toFieldSet()
    }
    ...
    private fun List<CompetitionEntity>.toIdNameSet(): Set<Pair<Int, String>> {
        return map { it.id to it.name }.toSet()
    }

    private fun List<CompetitionEntity>.toFieldSet(): Set<CompetitionFields> {
        return map {
            CompetitionFields(
                id = it.id,
                name = it.name,
